...
// // Phiên bản này ẩn số dư, mặc định hiển thị trong tin nhắn Số dư =0 

const CONFIG = {
  // ⚠️ THAY ĐỔI CÁC THÔNG TIN DƯỚI ĐÂY TRƯỚC KHI CHẠY
 ...
 * Hiển thị dữ liệu từ bảng tính lên giao diện Web
 */
function doGet() {
  try {
    ...
    `;
    return HtmlService.createHtmlOutput(html).setTitle("Timo Transaction Log");
  } catch (e) {
  ...
  try {
    ...
    threads.forEach(thread => {
      ...
      messages.forEach(msg => {
        ...
        // Regex bóc tách dữ liệu cụ thể cho Timo
        ...
        const balanceMatch = body.match(/Số dư hiện tại:\s+([\d,.]+)\s*VND/i);
        ...
        if (typeMatch && amountMatch) {
          ...
          const desc = descMatch ? descMatch[1].trim() : "Giao dịch qua Timo";
...
function sendToTelegram(text) {
  if (CONFIG.TELEGRAM_TOKEN === 'YOUR_BOT_TOKEN_HERE') return false;
